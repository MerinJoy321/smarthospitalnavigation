...
class FloorTransition {
  FloorTransition({
    ...
    required this.transitionType, // 'elevator' or 'stairs'
  });

  final int fromFloor;
  final int toFloor;
  final LocationNode transitionNode;
  final String transitionType;
}

class MultiFloorRoute {
  MultiFloorRoute({
    required this.floorRoutes,
    required this.transitions,
    required this.totalDistanceMeters,
  });

  final Map<int, NavigationRoute>
  floorRoutes; // floor number -> route on that floor
  final List<FloorTransition> transitions;
  final double totalDistanceMeters;

  /// Get the current floor route (for single floor display)
  NavigationRoute? getRouteForFloor(int floor) => floorRoutes[floor];

  /// Get all floors involved in this route
  List<int> get involvedFloors => floorRoutes.keys.toList()..sort();
}
