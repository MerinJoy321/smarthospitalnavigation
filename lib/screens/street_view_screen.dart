...
class StreetViewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Visual corridor guide'),
      ),
      body: steps.isEmpty
          ? const Center(
              child: Text('No steps for this route.'),
            )
          : Column(
              children: [
                Expanded(
...
class _CorridorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Ceiling.
    canvas.drawRect(
      Rect.fromLTRB(0, 0, size.width, midY * 0.2),
      ceiling,
    );
    ...
    canvas.drawCircle(
      Offset(
        doorRect.center.dx + doorRect.width * 0.25,
        doorRect.center.dy,
      ),
      3,
      handlePaint,
